...
function normaliseGroundTruthSample(sample) {
  return {
    ...
    source: 'ground_truth',
    teacher: `ground_truth:${sample.sourceType}`,
...
async function main() {
  ...
  const groundTruthDataset = loadGroundTruthDataset(datasetDir);
  const approvedSamples = groundTruthDataset.samples.filter((sample) => allowedStatuses.has(sample.reviewStatus));
  const importedTrainingSamples = approvedSamples.map(normaliseGroundTruthSample);
  ...
  for (const sample of importedTrainingSamples) {
    const key = trainingKey(sample);
    if (importedIndexByKey.has(key)) {
      throw new Error(`Duplicate ground-truth merge key: ${key}`);
    }
    importedIndexByKey.set(key, sample);
  }
  ...
  console.log(`Merged ground-truth dataset ${groundTruthDataset.manifest.datasetId} into ${curatedPath}`);
  console.log(`Imported approved samples: ${importedTrainingSamples.length}`);
